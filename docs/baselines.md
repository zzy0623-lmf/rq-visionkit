...
- [ ] RK3506 官方 YOLOv3 示例时延/内存（板到手当天，T0.4；板子无法运行，未实测）
- [x] PC 仿真：主 Demo 模型 FP32 vs INT8 时延/mAP（T1.4/T1.7，已补见上表 2026-09-22：FP32 10.29ms / mAP 6.37%，INT8 11.61ms / 6.71%）
- [x] 论文 MobileNetV2：FP32 基线 84.15% → NCNN FP32 83.61%；INT8 17.49%（量化失败未达 ≤2%，已按方案 1 以 FP32 验收，详见上表）
- [ ] RK3506 实测：自定义模型时延/内存（T2.2；板子无法运行，未实测）
